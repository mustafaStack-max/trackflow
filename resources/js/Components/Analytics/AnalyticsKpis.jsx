...
function AmountBadge({ amount, label = 'عن الفترة السابقة' }) {
    ...
    return (
        <span className={`${F.mono} text-[0.62rem]`} style={{ color }}>
            <span dir="ltr">{`${good ? '+' : '-'}${fmtMAD(Math.abs(amount))} MAD`}</span> {label}
        </span>
    );
}
...
function SavingsSub({ current, previous }) {
    ...
    return (
        <span className={`${F.mono} text-[0.62rem]`} style={{ color }}>
            <span dir="ltr">{`${diff >= 0 ? '▲' : '▼'} ${Math.abs(diff).toFixed(1)}`}</span> نقطة عن الفترة السابقة
        </span>
    );
}
...
function MiniStat({ icon: Icon, label, value, sub, color = C.t1 }) {
    return (
        <div
            ...
            style={{ background: C.card2, borderColor: C.b }}
        >
            <div
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                ...
                <KpiBox
                    ...
                    tag="NET"
                    value={<span dir="ltr">{`${net >= 0 ? '+' : '-'}${fmtMAD(Math.abs(net))} MAD`}</span>}
                    label="صافي الفترة"
